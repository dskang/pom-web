...
exports.connect_chatter = function  (current_socket) {
	...
	queue.addItem({socket: current_socket});
	current_socket.emit('waiting', {message: 'Waiting for partner to join.'});
    }
    else {
	partner =  queue.getItem();
	current_socket.emit('connected', {message: 'Connected! Go ahead and start chatting.'});
	partner.socket.emit('connected', {message: 'Connected! Go ahead and start chatting.'});
